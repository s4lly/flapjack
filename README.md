...
- Time announcements on a configurable cadence:
  ...
  - off
- **Three ways to be told, each switchable on its own** — the "Convey with" toggles in Settings. **Voice** speaks the time aloud ("ten oh-seven PM"); **Notification** posts it as a macOS notification; **Flying banner** sends a little plane — or a cat waving hello — carrying the time across your screen. Turn on any, all, or none. Notifications are off until you ask for them — flipping the toggle on is what requests permission.
- **Flying banner** (off by default): at each cadence tick a plane tows a banner reading the time across whichever desktop you're looking at — over your windows, the Dock and the menu bar — from a random side at a random height, once, in about seven seconds, and then it's gone. It's a ghost: clicks go straight through it, there's nothing to dismiss. A **Style** picker beside the toggle swaps the plane for a cat that waves hello as it goes, carrying the same banner the same way. The **Preview** button in Settings sends one across right now. One thing macOS won't allow: while another app is in native full screen, the banner can't be shown on that Space, so a tick that lands there passes unseen.
